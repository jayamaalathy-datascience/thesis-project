...
class CustomResNet(nn.Module):
    ...
    def _replace_group_norm(self, module, normalization):
        for name, child in module.named_children():
            if isinstance(child, nn.GroupNorm):
                setattr(module, name, normalization(child.num_features))
            else:
                self._replace_group_norm(child, normalization)
...
if __name__ == "__main__":
    ...
    model = CustomResNet(model_type='resnet50', num_classes=10, normalization=nn.LayrNorm)
    print(model)
