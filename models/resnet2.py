...
class CustomResNet(nn.Module):
    ...
    """
    def __init__(self, model_type, num_classes, normalization=nn.LayerNorm):
        super(CustomResNet, self).__init__()
        self.backbone = self._get_resnet_model(model_type)
        
        # Replace all BatchNorm layers
        self._replace_layer_norm(self.backbone, normalization)
    ...
    def _replace_layer_norm(self, module, normalization):
        for name, child in module.named_children():
            if isinstance(child, nn.LayerNorm):
                setattr(module, name, normalization(child.num_features))
            else:
                self._replace_layer_norm(child, normalization)
